...
test("Moldavite: renderReportHtmlPage uses Moldavite HTML V2 (radar, owner, graph data)", () => {
  ...
  assert.ok(html.includes('id="mv2-owner-h">คุณ</h2>'));
  assert.ok(html.includes('class="mv2-owner-zodiac"'));
  assert.ok(html.includes("ราศีมีน"));
  ...
  assert.ok(html.includes("เทียบจากวันเกิด"));
  ...
  assert.ok(html.includes('class="mv2-gauge-kicker">เสริมแรง</span>'));
  assert.ok(html.includes('class="mv2-gauge-kicker">ระวังจังหวะ</span>'));
  assert.ok(html.includes('class="mv2-gauge-kicker">โทนหิน</span>'));
  ...
  assert.ok(html.includes("มิติชีวิต"));
  assert.ok(html.includes('class="mv2-bars"'));
  ...
  assert.ok(!html.includes('class="mv2-life-grid"'));
  assert.ok(html.includes("หมายเหตุ"));
  ...
  assert.ok(html.includes("ไม่แทนคำแนะนำการแพทย์"));

...
test("renderAmuletReportV2Html: renders sacred amulet shell", () => {
  ...
  assert.ok(html.includes("mv2-timing-cta--inline"));
  assert.ok(html.includes('class="mv2-timing-cta-btn"'));
  assert.ok(html.includes("ดูวิธีคำนวณจังหวะเสริมพลัง"));
  assert.ok(
    html.includes("อธิบายว่าทำไมอาจารย์ถึงแนะนำวันนี้และช่วงเวลานี้"),
  );
  assert.ok(
...
test("renderAmuletReportV2Html: library mini box when sacredAmuletLibrary provided", () => {
  const payload = {
    reportId: "r1",
    publicToken: "t",
    scanId: "s1",
    ...
    reportVersion: "1",
    object: { objectImageUrl: "https://example.com/thumb.jpg" },
    summary: {
      energyScore: 8.9,

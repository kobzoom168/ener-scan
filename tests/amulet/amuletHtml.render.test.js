...
test("renderAmuletReportV2Html: renders sacred amulet shell", () => {
  ...
  assert.ok(html.includes("จังหวะและเพดานของชิ้นนี้"));
  assert.ok(html.includes('class="mv2-card mv2-card--guide"'));
  assert.ok(html.includes("ใช้เด่นสุด"));
  assert.ok(html.includes("ถ้าทำต่อเนื่อง"));
  assert.ok(html.includes("เพดานรอบนี้"));
  assert.ok(html.includes("ใช้ตามวันเวลาแนะนำ"));
  assert.ok(html.includes("เกรดปัจจุบัน · จังหวะซ้ำ"));
  assert.ok(html.includes("โบนัสจังหวะประมาณ"));
  assert.ok(html.includes("คะแนนหลักของรายงานไม่เปลี่ยน"));
  assert.ok(html.includes("mv2-guide-buff-table"));
  assert.ok(html.includes("เป็นโบนัสพิเศษ ไม่ใช่ base rule ของทุกชิ้น"));

...
test("renderAmuletReportV2Html: renders sacred amulet shell", () => {
  ...
  assert.ok(html.includes("จังหวะเกิดเดือนมิถุนายน"), "soft owner rhythm line under object-reaction title");
  assert.ok(html.includes("ชิ้นนี้ทำให้คุณเด่นแบบไหน"), "object-reaction card title");
  assert.ok(html.includes("ส่งกับคุณตรงสุด"));
  assert.ok(html.includes("เวลาใช้ชิ้นนี้"));
  assert.ok(html.includes("มุมที่ควรค่อย ๆ ไป"));

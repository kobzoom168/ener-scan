#!/usr/bin/env node
/**
 * Crystal artifact ownership map + review responsibility model.
 * See `docs/ops/crystal-artifact-ownership-model.md`.
 */
import fs from "fs";
...
const model = buildCrystalArtifactOwnershipModel(payload, {
  generatedAt: payload.generatedAt,
});

if (writeTable) {
  const tablePath = path.resolve(process.cwd(), "docs/ops/tables/crystal-artifact-ownership-model.json");
  fs.mkdirSync(path.dirname(tablePath), { recursive: true });
  const table = buildCrystalArtifactOwnershipModelTable();
...
if (format === "json") {
  ...
  process.stdout.write(renderCrystalArtifactOwnershipModelMarkdown(model));
} else {

    ...
    const response = await Promise.race([
      withOpenAi429RetryOnce(() =>
        openai.responses.create({
          user: "imageForensic.screen_check",
...
export const FORENSIC_RETRY_TEXTS = [
  "ภาพนี้ไม่ได้ถ่ายจากของจริง\nถ่ายสดวางบนพื้นเรียบแล้วส่งใหม่",
  "ภาพนี้ผ่านหน้าจอมาก่อน อ่านคลาดเคลื่อน\nถ่ายจากองค์จริงส่งมาใหม่",
];
...
export const CHALLENGE_REQUEST_TEXTS = [
  ...
  "ถ่ายชิ้นเดิมมุมเดิม ใช้นิ้วโป้งแตะชิ้นงานให้เห็นในรูป",
  "ถ่ายชิ้นเดิมมุมเดิม เอานิ้วโป้งแตะชิ้นงานให้เห็นในรูป",
  "ถ่ายรูปใหม่มุมเดิม ให้นิ้วโป้งแตะบนชิ้นงาน",
  "ถ่ายชิ้นเดิมมุมเดิมสด ๆ ใช้นิ้วโป้งแตะชิ้นงานให้เห็น",
  "ถ่ายชิ้นเดิมมุมเดิม เอานิ้วโป้งแตะชิ้นงานไว้ในรูปเลย",
];

/** รูปที่สองไม่ใช่ชิ้นเดียวกัน (หรือไม่มีของจริงให้ถ่าย) — ตอบแบบมีบารมี ไม่เผาขาด */
export const CHALLENGE_FAILED_TEXTS = [
  "สองรูปนี้คนละชิ้น\nถ่ายชิ้นเดิมมุมเดิมพร้อมนิ้วโป้งแตะ ส่งใหม่เลย",
];

/** ชิ้นเดียวกันจริงแต่ยังไม่เห็นนิ้วแตะ — ให้โอกาสถ่ายซ้ำ (คีย์ challenge ยังไม่หมดอายุ) */
export const CHALLENGE_NO_THUMB_TEXTS = [
  "ชิ้นเดิมถูกต้อง แต่รูปนี้ยังไม่เห็นนิ้วแตะที่ชิ้นงานเลย ถ่ายอีกรอบ มุมเดิม แล้วใช้นิ้วโป้งแตะค้างไว้ให้เห็นชัด ๆ ส่งมา",
];
...
export async function verifyChallengeThumbTouch(imageBase64) {
  ...
  try {
    const response = await Promise.race([
      withOpenAi429RetryOnce(() =>
        openai.responses.create({
          user: "imageForensic.thumb_touch",
          model: env.IMAGE_FORENSIC_MODEL,
          temperature: 0,
          input: [
            {

 ...
 *  persona 2 ชั้น (11 ส.ค.): คนตอบคือแอดมิน รับเรื่องแล้วส่งต่ออาจารย์ */
// Codex M7 (12 ส.ค.): ห้ามสัญญาเวลาเกินจริง — บอก 2-3 นาที หรือกลาง ๆ ไม่พูด "แปปเดียว"
// flow-role audit 26 ส.ค. (Codex H): ack ห้ามสัญญาเวลา/ผล — เพราะรายงานอาจถูกเกตเก็บข้อมูลยึดรอลูกค้ากรอก
// (เคส 7/10 รอ 15–36 นาที) · คงโทนเดิม ตัดเฉพาะ "2-3 นาที / ไม่เกิน 3 นาที / เดี๋ยวผลมา"
const PRE_SCAN_ACK_VARIANTS = [
  "รับแล้วครับ เดี๋ยวส่งให้อาจารย์เลย",
  "ได้รูปแล้วครับ ส่งให้อาจารย์ต่อเลยครับ",
  "รับแล้วนะครับ เข้าคิวอาจารย์ให้ละ",
  "มาละครับ เดี๋ยวอาจารย์ดูให้",
  "ได้ละครับ ส่งต่อให้อาจารย์เลย",
  "รับแล้วครับ อาจารย์กำลังดูให้",
  "เข้าคิวให้ละครับ",
  "ส่งให้อาจารย์แล้วครับ",
  "รับรูปแล้วครับ",
  "ได้รูปละครับ เดี๋ยวอาจารย์เพ่งให้",
];
...
export const MULTI_IMAGE_WAIT_TEXT =
  "ขอทีละ 1 รูปนะครับ\nชิ้นแรกอาจารย์กำลังดูอยู่ เดี๋ยวผลออกแล้วค่อยส่งชิ้นถัดไป\nถ้าชิ้นเดียวกันหลายมุม เลือกมุมที่ชัดสุดรูปเดียวพอครับ";

/** ครั้งที่ 2+ ภายใน 6 ชม. → ดุ. */
const MULTI_IMAGE_WAIT_TEXT_STERN =
  "ชิ้นแรกยังอยู่ในคิวครับ รูปที่ส่งเพิ่มยังไม่ถูกหักสิทธิ์\nรอผลชิ้นแรกก่อน แล้วค่อยส่งชิ้นถัดไปทีละรูปนะครับ";
...
export async function ingestScanImageAsyncV2({
  ...
  try {
    ...
    } else if (bind && (bind.status === "stale_after_prior_job" || bind.status === "stale_check_failed" || bind.status === "source_changed")) {
      // ข้อมูลก่อนรูปเป็นของรูปก่อนหน้า / ตรวจไม่ได้ / source ถูกเขียนทับระหว่าง precheck → ไม่ MOVE ไม่ให้ติดรูปนี้ · gate ถามตามปกติ
      console.log(JSON.stringify({ event: "PRE_SCAN_OBJECT_INFO_STALE_DISCARDED", ...base(), jobIdPrefix: idPrefix8(jobRow.id), status: bind.status, priorJobIdPrefix: bind.priorJobIdPrefix || null, message: bind.message || null }));

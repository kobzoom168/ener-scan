...
export async function generateSmartRejectionText(p) {
  ...
  try {
    ...
    const prompt = [
      ...
      escalatePhase
        ? "ลูกค้าเริ่มเหนื่อยแล้ว: ห้ามพูดซ้ำแนวเดิม ให้เปลี่ยนวิธีช่วยแบบเจาะจง (เช่น วางบนกระดาษขาว ถอยกล้องออกนิด แตะจอให้โฟกัสที่ตัววัตถุก่อนถ่าย ปิดแฟลช) และให้กำลังใจสั้น ๆ ว่าใกล้ได้แล้ว"
        : p.imageDark
      ...
      "ตอบสั้นแบบคนพิมพ์แชทจริง 1 ถึง 2 ประโยคจบ (ตัวอย่างความยาวที่ดี: ขอภาพสว่างกว่านี้หน่อยครับ มองไม่เห็น) ไม่ใส่อีโมจิ ไม่มีเครื่องหมายคำพูด ห้ามใช้เครื่องหมายขีดคั่นประโยค (— – หรือ -) คนจริงไม่พิมพ์แบบนั้น ตอบเฉพาะข้อความที่จะส่งให้ลูกค้าเท่านั้น",
    ].join("\n");

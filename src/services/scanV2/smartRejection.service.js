...
export async function generateSmartRejectionText(p) {
  ...
  try {
    ...
    const prompt = [
      ...
      "ตอบสั้นแบบคนพิมพ์แชทจริง 1 ถึง 2 ประโยคจบ (ตัวอย่างความยาวที่ดี: ขอภาพสว่างกว่านี้หน่อย มองไม่เห็น) ไม่ใส่อีโมจิ ไม่มีเครื่องหมายคำพูด ห้ามใช้เครื่องหมายขีดคั่นประโยค (— – หรือ -) คนจริงไม่พิมพ์แบบนั้น ตอบเฉพาะข้อความที่จะส่งให้ลูกค้าเท่านั้น",
    ].join("\n");

/**
 * Sacred amulet lane: summary-first Flex (grey layers + gold accents). คล้ายธีมดำเดิมที่แยก “มิติ”
 * ด้วยแผงเข้ม/ขอบ — ใช้โทนเทาอ่อน + การ์ดยก + เส้นขอบ (LINE ไม่รองรับ gradient/shadow จึงใช้ชั้นสี + border).
 *
...
const AMULET_VISIBLE_LABEL_FALLBACK = "พระ/เทวรูป/เครื่องราง";
...
/** Outer bubble / hero: เทากลาง (แท่นวาง). */
const FLEX_CARD_BG = "#e9ecef";
/** การ์ดคู่ + บล็อกเด่นสุด: อ่อนกว่าพื้นเล็กน้อย + ขอบ = รู้สึกยกขึ้น */
const FLEX_BOX_BG = "#f4f6f8";
const FLEX_BOX_BORDER = "#c5ccd6";
/** Pill meter track (บนพื้นเทา). */
const LIFE_AREA_BAR_TRACK_BG = "#d8dee6";
...
/** ปิลล์สรุปท้าย bubble (เทียบ HTML .mv2-gsum-row) */
const FLEX_PILL_BG = "#fdfbf7";
const FLEX_PILL_BORDER = "#e8dcc4";
const FLEX_PILL_LEAD_BG = "#fcf8f3";
const FLEX_PILL_LEAD_BORDER = "#e0d4bc";
const FLEX_PILL_LABEL = "#78716c";
const FLEX_PILL_VALUE_SECOND = "#1c1917";
...
/** Flex tagline: max Thai chars for “เด่น{มิติ}” segment after `พระ/เทวรูป/เครื่องราง · `. */
...
function parseFitLineToSummaryBlock(raw) {
  const s = String(raw || "").trim();
  if (!s) return null;
  let m = s.match(/^ตอนนี้เด่นสุด\s*:\s*(.+)$/s);
...
/**
 * แถวปิลล์เดียว (label ซ้าย / ค่าขวา) — เฉพาะบล็อกล่างของ bubble
 * @param {boolean} isLead แถวพลังเด่น → ตัวเลขทอง
 */
function createGsumPillRow(label, value, isLead) {
  return {
    type: "box",
    layout: "horizontal",
    ...
    paddingEnd: "14px",
    cornerRadius: "xxl",
    backgroundColor: isLead ? FLEX_PILL_LEAD_BG : FLEX_PILL_BG,
    borderWidth: "1px",
    borderColor: isLead ? FLEX_PILL_LEAD_BORDER : FLEX_PILL_BORDER,
    contents: [
      {
        type: "text",
        text: label,
        size: "xxs",
        color: FLEX_PILL_LABEL,
        flex: 0,
        wrap: false,
      },
      {
        ...
        size: "sm",
        weight: "bold",
        color: isLead ? AMULET_ACCENT : FLEX_PILL_VALUE_SECOND,
        flex: 1,
        align: "end",
        wrap: true,
        maxLines: 2,
...
function buildAmuletFlexTaglineDisplay(mv) {
  ...
  return `พระ/เทวรูป/เครื่องราง · เด่น${dim}`;
}

...
export function buildPaymentRequiredFlex({ usedScans = 0, freeLimit = 2 } = {}) {
  ...
  return createBaseBubble({
    ...
    bodyContents: [
      createCard(
        "สถานะตอนนี้",
        `ใช้ไปแล้ว ${used} ครั้ง\nโอนแล้วส่งสลิปในแชทนี้ เดี๋ยวเราตรวจและเปิดสิทธิ์ให้ตามรายการค่าครู`,
        {
          backgroundColor: "#171717",
          borderColor: "#242427",
        }
  ...
  return createBaseBubble({
    ...
    bodyContents: [
      createCard(
        ...
        [
          `สิทธิ์ฟรีที่มี${freeLimit ? ` (${freeLimit} ครั้ง)` : ""}ใช้ครบแล้ว รายงานเดิมยังเปิดดูได้`,
          "",
          ...
          "• คำแนะนำเฉพาะตัว",
          "",
          "✨ ค่าครูแนะนำ: สิทธิ์ตามที่แจ้งหลังอนุมัติ (เช่น ใช้ในช่วง 24 ชม.)",
          `💎 ${priceLine}`,
        ].join("\n"),
        {
          backgroundColor: "#171717",

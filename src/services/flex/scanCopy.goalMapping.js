...
import {
  ...
  GOAL_CLUSTER_PRIMARY,
  GOAL_HEADLINE_TEMPLATE,
...
export function deriveGoalMapping({
  ...
  if (!me || me === "-" || !scoreOk) {
    return {
      ...
      goalHeadline: GOAL_UNCLEAR.headline,
      goalSecondary: GOAL_UNCLEAR.secondary,
  ...
  const cluster =
    GOAL_CLUSTER_PRIMARY[energyType] ||
    GOAL_CLUSTER_PRIMARY[ENERGY_TYPES.BOOST];
  const template = GOAL_HEADLINE_TEMPLATE[clarity];
  const goalHeadline = template.replace("{cluster}", cluster);

  const notPrimaryFor = GOAL_NOT_PRIMARY_HINT[energyType];

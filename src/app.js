...
async function handleEvent(event) {
  ...
  try {
    ...
    return client.replyMessage(replyToken, {
      type: "text",
      text: "⚠️ เกิดข้อผิดพลาด กรุณาลองใหม่นะครับ",
    });
  }
}
      ...
      messages: [
        {
          role: "system",
          content:
            "ตรวจว่าวัตถุในภาพเป็นประเภทอะไร (พระเครื่อง เครื่องราง คริสตัล) ถ้าใช่ให้ตอบ SUPPORTED: <ประเภท> ถ้าไม่ใช่ให้ตอบ NOT_SUPPORTED",
        },
        {
          role: "user",
          content: [
...
async function analyzeDeepScan({ base64Image, birthdate, objectTypeHint, rank }) {
  try {
    ...
    return (
      ...
      "อาจารย์ Ener ไม่สามารถอ่านพลังได้ในขณะนี้"
    );
  } catch (err) {
    console.error("analyzeDeepScan error:", err);
    return "⚠️ ระบบวิเคราะห์เกิดข้อผิดพลาด กรุณาลองใหม่";
  }
}

...
const DAILY_TONES = [
  "โทนอ่านชิ้นนี้เหมาะเทียบหลายองค์แบบไม่เร่ง",
  "โทนนิ่งพอดี เห็นความต่างชิ้นละชิ้นชัดขึ้น",
  "อ่านแบบใจเย็น ให้เวลารายละเอียดเล็ก ๆ",
  "เน้นตัดสินใจด้วยเหตุผลมากกว่าอารมณ์วูบเดียว",
  "เหมาะจัดลำดับว่าอะไรสำคัญกว่าในคลัง",
  "สังเกตความต่างระหว่างชิ้นอย่างใจเย็น",
  "เทียบหลายชิ้นแล้วค่อยเลือกตัวที่ใช่กับจังหวะคุณ",
];

const DAILY_SCAN_HINTS = [
  "สแกนเพิ่ม เทียบสายเมตตาหรือบารมีก่อน",
  "ลองเทียบโทนคุ้มครองหรือหนุนดวงใกล้กัน",
  "ยึด “เข้ากับคุณ” เป็นตั้งต้นมากกว่าแค่คะแนนสูงสุด",
  "ดูทั้งพลังเด่นกับด้านที่ส่งกับคุณคู่กัน",
  "ลองสายงานเฉพาะหรือโชคลาภคู่ของเดิม",
  "ใช้รูปและวันเกิดเดิมให้ผลเทียบตรง",
];
...
export function buildTodayObjectBoostLine(p) {
  ...
  if (c >= 76) {
    return `ชิ้นนี้ช่วยดัน${shortAlign}ได้ดีเมื่อเทียบกับจังหวะคุณ`;
  }
  if (c >= 62) {
    const tensionFrag =
      tk !== ak && shortTension
        ? ` · มุม${shortTension}ของชิ้นนี้เหมาะใช้เป็นจุดเทียบมากกว่าจุดเด่นหลัก`
        : "";
    return `ชิ้นนี้ส่งกับ${alignL}พอดี · ค่อย ๆ เทียบชิ้นอื่นในสายเดียวกันจะเห็นชัดขึ้น${tensionFrag}`;
  }
  return `ชิ้นนี้ยังไม่เด่นสุด ลองเทียบชิ้นที่ส่งทาง${shortPeak}เพิ่ม · ถ้าเทียบหลายชิ้น ให้ดูด้าน${shortTension}เป็นจุดอ้างอิงไปก่อน`;
}

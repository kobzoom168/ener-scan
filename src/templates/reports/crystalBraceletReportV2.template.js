...
const CB2_FOOTER_EXPLAINER =
  "กำไลเส้นเดียวกัน แต่ละคนรับพลังได้ไม่เท่ากัน เพราะขึ้นกับวันเกิดและจังหวะชีวิตของผู้สวมด้วย Ener Scan จึงอ่านผลเฉพาะตัวคุณ";
...
const CB_GRAPH_SUMMARY_SUB = {
  charm_attraction: {
    primary:
      "กำไลเส้นนี้ส่งพลังด้านเสน่ห์และแรงดึงดูดชัดที่สุดในช่วงนี้",
    secondary:
      "รองลงมาคือเสน่ห์ ช่วยหนุนภาพลักษณ์และความน่าเข้าหา",
    align:
      "ช่วงนี้จังหวะคุณรับด้านเสน่ห์ได้ดีเป็นพิเศษ",
  },
  money: {
    primary:
      "กำไลเส้นนี้ส่งพลังด้านการเงินชัดที่สุดในช่วงนี้",
    secondary:
      "รองลงมาคือการเงิน ช่วยหนุนความคล่องตัวเรื่องรายรับ",
    align:
      "ช่วงนี้จังหวะคุณรับด้านการเงินได้ดีเป็นพิเศษ",
  },
  career: {
    primary:
      "กำไลเส้นนี้ส่งพลังด้านการงานชัดที่สุดในช่วงนี้",
    secondary:
      "รองลงมาคือการงาน ช่วยหนุนความต่อเนื่องในสิ่งที่ทำ",
    align:
      "ช่วงนี้จังหวะคุณรับด้านการงานได้ดีเป็นพิเศษ",
  },
  luck: {
    primary:
      "กำไลเส้นนี้ส่งพลังด้านโชคลาภและจังหวะเปิดทางชัดที่สุดในช่วงนี้",
    secondary:
      "รองลงมาคือโชคลาภ ช่วยหนุนโอกาสใหม่ที่เข้ามา",
    align:
      "ช่วงนี้จังหวะคุณรับด้านโชคลาภได้ดีเป็นพิเศษ",
  },
  intuition: {
    primary:
      "กำไลเส้นนี้ส่งพลังด้านเซ้นส์และการตัดสินใจชัดที่สุดในช่วงนี้",
    secondary:
      "รองลงมาคือเซ้นส์ ช่วยให้อ่านจังหวะและตัดสินใจได้ไวขึ้น",
    align:
      "ช่วงนี้จังหวะคุณรับด้านเซ้นส์ได้ดีเป็นพิเศษ",
  },
  love: {
    primary:
      "กำไลเส้นนี้ส่งพลังด้านความรักและความสัมพันธ์ชัดที่สุดในช่วงนี้",
    secondary:
      "รองลงมาคือความรัก ช่วยหนุนความอ่อนโยนกับคนสำคัญ",
    align:
      "ช่วงนี้จังหวะคุณรับด้านความรักได้ดีเป็นพิเศษ",
  },
};
 ...
 */
function buildCrystalBraceletEnergyTimingHtml(hr, publicToken = "") {
...
function buildCrystalBraceletGraphSummaryBarsHtml(
  ...
  const peakColor = CB_RING_COLORS[primaryAxis] || "#b8871b";
  const alignColor = CB_RING_COLORS[alignAxisKey] || "#8f6710";
  ...
  return `
    <section class="cb2-card cb2-lib" aria-labelledby="cb2-lib-h">
      <style>
        ...
        .cb2-lib-ax-slide{flex:0 0 min(80%,240px);scroll-snap-align:start;scroll-snap-stop:always;border-radius:.8rem;border:1px solid rgba(184,135,27,.30);background:rgba(184,135,27,.07);padding:.6rem;box-sizing:border-box;text-align:center;animation:cb2LibRise .5s ease both}
        ...
        .cb2-lib-ax-btn{display:block;text-align:center;padding:.5rem;font-size:.82rem;font-weight:800;border-radius:999px;text-decoration:none;color:#04121d;background:linear-gradient(165deg,#8f6710,#b8871b 55%,#8f6710)}
        .cb2-lib-ax-btn--off{opacity:.45;background:rgba(184,135,27,.15);color:inherit}
...
export function renderCrystalBraceletReportV2Html(payload, options = {}) {
  ...
  // Older payloads baked "กำไลหินคริสตัล" — display as just "กำไล" (many bracelet kinds).
  const simplifyBraceletWord = (t) => String(t || "").split("กำไลหินคริสตัล").join("กำไล");
  const headline = simplifyBraceletWord(String(fs?.headline || "").trim()) || "กำไล";
  const tagline = simplifyBraceletWord(String(fs?.tagline || "").trim()) || "กำไล · อ่านจากพลังรวม";
  ...
  const ogDescription =
    "ดูรายงานพลังกำไลจาก Ener Scan พร้อมเรดาร์ สรุปจากกราฟ และจังหวะเสริมพลัง";
  ...
  const energyTimingHtml = buildCrystalBraceletEnergyTimingHtml(
    hr,
...
<head>
  ...
  <style>
    :root {
      /* Feminine light theme (ชมพูพาสเทล-ม่วงลาเวนเดอร์) — crystal bracelets skew female */
      --cb2-bg: #f6f6f4;
      --cb2-card: #ffffff;
      --cb2-card2: #f2f2ef;
      --cb2-border: rgba(100,92,82,0.12);
      --cb2-text: #241c12;
      --cb2-sub: #5a4a38;
      --cb2-muted: #7a6a58;
      --cb2-accent: #b8871b;
      --cb2-accent2: #8f6710;
      --cb2-card-shadow: 0 2px 12px rgba(184,135,27,0.16), 0 0 0 1px rgba(100,92,82,0.05);
      --cb2-gsum-bg: #f2f2ef;
      --cb2-gsum-border: rgba(100,92,82,0.10);
      --cb2-gsum-lead-bg: rgba(184,135,27,0.12);
      --cb2-gsum-lead-border: rgba(184,135,27,0.32);
      --cb2-gsum-k: #7a6a58;
      --cb2-gsum-v: #241c12;
      --cb2-gsum-v-lead: #b8860b;
    }
    * { box-sizing: border-box; }
    ...
    .cb2-hero-img { width: 100%; aspect-ratio: 4/3; border-radius: 16px; overflow: hidden; background: #f2f2ef; margin-bottom: 0.85rem; box-shadow: 0 4px 20px rgba(0,0,0,0.5); }
    ...
    .cb2-h1 { font-size: 1.4rem; font-weight: 800; margin: 0 0 0.2rem; line-height: 1.3; color: #241c12; letter-spacing: -0.01em; }
    ...
    .cb2-owner-card {
      ...
      border-left: 3px solid rgba(184,135,27, 0.55);
    }
    .cb2-owner-card--below-summary {
      margin-top: 1.15rem;
      padding-top: 0.65rem;
    }
    .cb2-owner-card h2 { font-size: 0.88rem; font-weight: 700; margin: 0 0 0.55rem; color: #241c12; }
    ...
    .cb2-owner-id { margin: 0; font-size: 0.8rem; font-weight: 600; line-height: 1.45; color: #241c12; }
    ...
    .cb2-owner-chip {
      ...
      border-radius: 999px;
      background: rgba(184,135,27, 0.10);
    ...
    .cb2-card--et {
      border-left-color: rgba(184,135,27, 0.55);
      margin-top: 1rem;
    }
    .cb2-card--et h2 {
      margin: 0;
    ...
    .cb2-et-strip--weekday .cb2-et-pill.is-active .cb2-et-pill-shape {
      background: linear-gradient(165deg, #e8c547 0%, #b8871b 45%, #8f6710 100%);
      border: 1px solid rgba(186, 230, 253, 0.55);
      box-shadow:
        0 0 0 1px rgba(125, 211, 252, 0.35),
        0 0 16px rgba(184,135,27, 0.22),
    ...
    .cb2-et-slot.is-active .cb2-et-slot-bar {
      width: 0.52rem;
      max-width: 48%;
      background: linear-gradient(180deg, #b8871b 0%, #b8860b 38%, #8f6710 100%);
      border: 1px solid rgba(165, 220, 252, 0.5);
      box-shadow:
        0 0 0 1px rgba(184,135,27, 0.25),
        0 0 14px rgba(184,135,27, 0.2),
    ...
    .cb2-share-card {
      border-left-color: rgba(184,135,27, 0.45);
      margin-top: 1rem;
    }
    .cb2-share-card h2 {
      color: #8f6710;
    ...
    .cb2-share-btn--primary {
      background: linear-gradient(165deg, #8f6710, #b8871b 55%, #8f6710);
      border-color: rgba(184,135,27, 0.5);
      color: #04121d;
    }
    ...
    .cb2-strip-v { font-size: 1.1rem; font-weight: 800; color: #241c12; margin-top: 0.25rem; line-height: 1; }
    ...
    .cb2-radar-legend-swatch--dashed {
      width: 18px;
      height: 0;
      border-top: 2px dashed #8f6710;
    }
    .cb2-radar-legend-swatch--dot {
      width: 10px;
      height: 10px;
      border-radius: 999px;
      background: rgba(120,108,84, 0.45);
      border: 2px solid rgba(184,135,27, 0.7);
    ...
    .cb2-gsum-bar-row--lead { border-bottom-color: rgba(184,135,27,0.18); }
    .cb2-gsum-bar-k {
      flex: 0 0 5.5rem;
      font-size: 0.68rem;
      font-weight: 600;
    ...
    .cb2-gsum-bar-track {
      ...
      background: rgba(100,92,82,0.10);
      overflow: hidden;
      min-width: 0;
    }
    ...
    .cb2-em-cta { margin-top: 0.9rem; padding: 0.75rem 0.8rem; border-radius: 12px;
      background: rgba(184,135,27,0.08); border: 1px solid rgba(184,135,27,0.25); }
    ...
    .cb2-em-cta-btn { display: block; text-align: center; padding: 0.7rem; border-radius: 999px;
      ...
      background: linear-gradient(165deg, #e8c547, #b8871b 55%, #8f6710); }

    /* ── Disclaimer ── */
    .cb2-disclaimer { margin-top: 1.25rem; padding: 0.8rem 1rem; border-radius: 12px; background: rgba(184,135,27,0.07); border: 1px solid rgba(184,135,27,0.20); font-size: 0.76rem; line-height: 1.55; color: #8f6710; }
    ...
    .cb2-meta-k { font-weight: 700; color: #241c12; opacity: 0.92; }
    .cb2-meta-v { text-align: right; font-variant-numeric: tabular-nums; max-width: 68%; }
    .cb2-meta-id { font-weight: 700; letter-spacing: 0.04em; color: #8f6710; }
    .cb2-trust-sources {
      margin: 0.85rem 0 0;
    ...
    .cb2-share-btn--secondary:hover { background: rgba(184,135,27, 0.1); }
    .cb2-share-btn--ghost {
    ...
    .pk-stickycta { position: fixed; left: 0; right: 0; bottom: 0; z-index: 60; display: flex; gap: 8px; padding: 10px 14px calc(10px + env(safe-area-inset-bottom)); background: rgba(255, 251, 253, 0.96); backdrop-filter: blur(8px); border-top: 1px solid rgba(184,135,27, 0.3); }

...
export function buildFollowWelcomeText() {
  ...
  return [
    ...
    `ถ่ายรูปส่งมา 1 ชิ้น อาจารย์เพ่งให้เลย ฟรีวันละ ${freeQ} ครั้ง`,
    ...
    "ไม่รับรูปตัดต่อ รูปแคปจอ รูปจากเน็ต หรือรูปที่มีตัวหนังสือแปะมา",
    "คลื่นพลังอ่านได้จากภาพถ่ายตรงจากของจริงเท่านั้น",
    "",
...
export function buildUnsupportedObjectText() {
  return [
    "รูปนี้อาจารย์ยังดูพลังให้ไม่ได้นะครับ 🙏",
    "",
    "ตอนนี้อาจารย์รับดู",
    "• พระเครื่อง / พระบูชา / เทวรูป / รูปปั้นสายมู",
    "• เครื่องราง / ตะกรุด / ผ้ายันต์",
    "• คริสตัล / หิน / กำไล",
    "• วัตถุสายพลังแบบชิ้นเดี่ยว",
    "",
    "ถ้าชิ้นที่ส่งมาเป็นของสายพลังอยู่แล้ว บางทีอาจารย์แค่มองจากมุมนี้ไม่ถนัด ลองถ่ายใกล้ขึ้นให้เห็นชิ้นเต็ม ๆ กลางภาพ แล้วส่งรูปมาใหม่ได้เลยครับ",
  ].join("\n");
}
...
export function getUnsupportedObjectReplyCandidates() {
  return [
    ...
    "ขอภาพพระ เทวรูป เครื่องราง ผ้ายันต์ หิน หรือกำไล ทีละชิ้นต่อรูปนะครับ ถ้าเป็นของสายพลังอยู่แล้วลองถ่ายใกล้ขึ้นอีกนิดแล้วส่งใหม่ได้เลย",
    "รูปนี้อาจารย์ยังอ่านไม่ถนัดครับ ถ้าเป็นพระ เครื่องราง หรือผ้ายันต์ ลองถ่ายให้เห็นชิ้นชัด ๆ กลางภาพแล้วส่งมาใหม่นะครับ",
  ];
}
...
export function buildPaymentQrSlipText() {
  ...
  if (/^\d{10,13}$/.test(ppId)) {
    lines.push(`หรือโอนพร้อมเพย์ ${ppId} (แตะค้างที่เลขเพื่อคัดลอก)`);
  }
  return lines.join("\n");
}
...
export function buildPaymentRejectedText({ reason = null } = {}) {
  ...
  if (r) {
    lines.push("รายละเอียดเพิ่มเติม:", r, "");
  }
  lines.push(

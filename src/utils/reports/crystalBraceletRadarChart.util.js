...
const STONE_STROKE = "#b8871b";
const STONE_FILL = "rgba(184,135,27,0.14)";
const OWNER_STROKE = "#8f6710";
const STONE_DOT_FILL = "#b8860b";
/** จุดรอง “เข้ากับคุณที่สุด” บนเส้นพลังกำไล — เทียบ moldavite mv2-radar-peak-compatibility */
const ALIGN_DOT_FILL = "rgba(120,108,84,0.9)";
const ALIGN_DOT_RING = "rgba(120,108,84,0.45)";
...
export function buildCrystalBraceletRadarChartSvg(axes, ownerAxisScores) {
  ...
  const gridRings = [SVG_R * 0.25, SVG_R * 0.5, SVG_R * 0.75]
    .map((r) => {
      return `<polygon points="${heptPoints(r)}" fill="none" stroke="rgba(100,92,82,0.18)" stroke-width="0.5"/>`;
    })
    .join("");
  ...
  const spokes = angles
    .map((ang) => {
      ...
      return `<line x1="${SVG_CX}" y1="${SVG_CY}" x2="${x2.toFixed(2)}" y2="${y2.toFixed(2)}" stroke="rgba(100,92,82,0.18)" stroke-width="0.4"/>`;
    })
    .join("");
  ...
  <circle cx="${pxf}" cy="${pyf}" r="3.2" fill="none" stroke="rgba(184,135,27,0.5)" stroke-width="0.8"/>

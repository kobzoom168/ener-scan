...
| 14 ส.ค. 2026 | Codex | `23bdf2e` telemetry release candidate | hung-main test ใช้ unresolved promiseจริง; targeted 6/6 ใน 0.32s; Responses scanไม่พบ create call ที่ขาด `user`; deepScan/fengShui tags แยกแล้ว | พร้อมให้เจ้าของเคาะ telemetry-only deploy; หลัง deployยืนยัน pro shadow=false และ OpenRouter tag แรกก่อนเริ่มนับ 2–3 วัน |
| 14 ส.ค. 2026 | Claude/เจ้าของ | `23bdf2e` telemetry rollout | **LIVE pro**: รายงาน web 200 + 3 containers healthy; pro shadow flag absent; smoke log `smoke.telemetryCheck` ได้ usage 6/1 tokens; ไม่แตะ cache/prompt/model/detail | ยืนยัน user tag ใน OpenRouter row/CSV แล้วเริ่มหน้าต่าง 2–3 วันจาก tagged real traffic; export CSV รอบใหม่เพื่อ cost/callSite + cache break-even |
| 14 ส.ค. 2026 | Codex | `e5250c5` chat-quality fixes | referral exact-command priority ถูกทิศ; marker false alarm และ prompt leak ถูกแตะ แต่ยังมี 3 gaps: referral error/disabled fall-through เข้า LLM, utility commands อยู่ก่อน registration gate, และไม่มี routing integration tests; marker regex กว้างเกิน metadata | แก้ก่อน pro: deterministic unavailable fallback, registration exception/priority contract, exact marker metadata, paywall+pending-image tests และ output validator/golden test สำหรับคำอวย |
| 14 ส.ค. 2026 | Codex | `0d52467` chat-quality follow-up | targeted 24/24; registration→utility order, terminal contract และ marker filter ถูกทิศ แต่ tone retry fail ยังส่งข้อความต้องห้ามเดิม; unavailable send fail ถูกกลืนและ log `_SENT`; ไม่มี routing invariant test | ก่อน pro: fail-closed tone sanitizer/fallback, truthful delivery telemetry + push fallback, source-order/single-owner invariant tests |
| 14 ส.ค. 2026 | Codex | `4abd7c6` tone/delivery follow-up | routing/delivery/invariants ผ่าน; targeted 23/23 แต่ sanitizer test เช็คเพียง banned regex จึงปล่อยภาษาเสีย: `อยู่ตามระดับที่ตัวเลขระบุดีแล้วครับ`, `เพิ่มเติมนที่ใช่เอง` | ห้าม pro จนแก้ longest-first/whole-clause sanitizer และล็อก exact readable outputs จากเคสจริง; Rich Menu เห็นชอบแทน “ถามแอดมิน” ด้วย “เปิดแอป Ener” |
| 14 ส.ค. 2026 | Codex | `6c9f654` sanitizer r3 + Rich Menu LIFF | sanitizer exact outputs ผ่านและ targeted 24/24; tone/routing fixes พร้อม pro หลัง staging smoke แต่ repo มีเพียง PNG generator ไม่มี rich-menu action manifest/deploy script ที่อ่าน `LIFF_ID` จึงยังพิสูจน์ auto staging/pro URI ไม่ได้ | ก่อน Rich Menu pro: commit manifest/deployer, env guard + expected LIFF host/id check, dry-run/action coordinate tests; เจ้าของทดสอบ 6 ปุ่ม staging |
| 14 ส.ค. 2026 | Claude/เจ้าของ | `f14b646` + `37beba4` / migration 052 / Rich Menu v4 | **LIVE pro**: `/myscans` + history card + LIFF pay status + telemetry; smoke token/page 200, security headers, DB grantsและ 3 containers healthy; pro menu `richmenu-210e1fbc…`, old menus retained for rollback | เฝ้า 2–3 วัน: unique opens, report CTR, broken images/token errors, pagination latency, pay conversion/state, duplicate-payment prevention; referral buttonยัง intentionally disabled |
| 28 ส.ค. 2026 | Claude/เจ้าของ | `241952d` flow-role → **LIVE pro** 08:14Z | LINE smoke staging: text 4 ข้อ deterministic AI=0 + QR ✓, เคส B challenge→retry rebind ✓, P0-A ack ✓, เคส A รูป 2 ไม่ BOUND ✓ (race ไม่พร้อมกัน) · Codex ใช้ไม่ได้ กบเคาะ GO เอง | เฝ้า Pro 24 ชม.; P1 dedup path ทิ้ง pre-scan info (BACKLOG); race พร้อมกันจริงยังพิสูจน์แค่ hermetic |

...
- [x] (P0-E flow-role 28 ส.ค.) objectInfoGate: path `SCAN_SHA256_DEDUP_HIT` (outbound ไม่มี report payload) และ `hasInfoForObject=true` ออกจาก gate ก่อน `consumeJobPreScanInfo` → ข้อมูลที่ลูกค้าพิมพ์ก่อนรูปค้างใน pre_job จนหมดอายุ (smoke 28 ส.ค. job aa15e0ab "หลวงพ่อคูณวัดบ้านไร่") — ควร consume+บันทึก (หรือ DEL พร้อม telemetry PRE_SCAN_OBJECT_INFO_DROPPED) ทุกทางออกของ gate · แยกรอบ ไม่ขยาย patch flow-role
- [ ] objectInfoGate × pre-scan: gate กำลังจะถามชิ้น A แต่ uid มี `objinfo:preprovided` สดที่ยังไม่ bind (ลูกค้าพิมพ์ข้อมูลไปก่อนถูกถาม ไม่กี่วินาที) → ควรใช้เป็นคำตอบของ A แทนการถาม (smoke 28 ส.ค. 09:08Z: พิมพ์ "พระปิดตา" 09:08:10 → gate ถาม 09:08:31 → ลูกค้ารู้สึกถามซ้ำ/ช้า) · ต้องนิยาม TTL ความสดและ tie-break กับรูปที่กำลังเข้าคิว · แยกรอบ (ออกแบบร่วม Codex)

## เยียวยา heldOnly 10 ราย (เปิด 3 ก.ย. 2026 — ตาม Codex: ตรวจทีละราย ห้ามส่ง/คืนสิทธิ์อัตโนมัติ)
ลูกค้าถูกถามข้อมูลชิ้นแล้วรายงานไม่เคยถูกปล่อย (บั๊กก่อน P0-F แก้แล้ว จะไม่เกิดซ้ำ) · ตรวจแล้วทุกรายไม่ถูกหักสิทธิ์ (path held ข้าม decrement ยุคนั้น + ledger=0) = ไม่มีความเสี่ยงชดเชยซ้ำเชิงเงิน
- **กลุ่ม paid 3 jobs** (08-08: 9116a8bc, 17363e69, 9c4d218a — ลูกค้าประจำจ่าย 20-26 รายการ): สแกนสำเร็จภายหลัง 8-31 ครั้งใน 48 ชม. = ได้ผลจากการส่งใหม่แล้ว + ไม่ถูกหักสิทธิ์รอบที่หาย → เสนอ**ไม่ต้องเยียวยา** (รายงานเก่า 3+ สัปดาห์ ส่งตอนนี้ UX แปลก)
- **กลุ่ม free ไม่เคยได้อะไรเลย 5 jobs / 4 คน**: U339c61e ×2 (08-11, 08-21) · Ubfacda4 (08-13) · U8ebe06d (08-24) · U57743ad ×2 (08-31, 09-01 **ล่าสุด ยัง active 09-01**) → เสนอเยียวยาเฉพาะ **U57743ad** (รายงานยังสด): ข้อความขอโทษ + ปล่อยรายงาน job ล่าสุด (210dddb9) · รายเก่า 2-3 สัปดาห์ให้กบเคาะว่าคุ้มปลุกไหม

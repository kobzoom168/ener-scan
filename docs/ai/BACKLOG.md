...
6. ~~C6 Synergy persona policy~~ ✅ ทำแล้ว 20 ส.ค. (intro+จัดชุด reply เสียงแอดมิน) — เหลือรีวิวข้อความ synergy อื่นถ้า Codex เจอเพิ่ม
7. **Gemini object-form 429 incident (แยก — Codex ข้อ 5)**: GEMINI_OBJECT_FORM_FAILED 22 ครั้ง/24 ชม. บน pro จาก Google direct quota — เลือกทาง: เพิ่ม quota / ย้าย second-opinion ไป OpenRouter / ปิด flag ชั่วคราว (GEMINI_OBJECT_FORM_ENABLED) · scan ไม่ล้ม (fail-open) แต่เกตธูป/ของใช้พิธีแม่นลดลง — ฟีเจอร์ไม่ใช่ bug · mixed voice ในข้อความเดียว = policy bug: แจ้งคลังครบ/URL/CTA = เสียงแอดมิน (exact "จัดชุด" ผ่าน sendNonScanReply speakerRole=admin) · คำแนะนำจับคู่ชิ้น = อาจารย์ เฉพาะเมื่อถูกถามหรือในรายงาน Synergy · เนื้อหาวิชาอยู่ในหน้ารายงาน

...
0.4 **Scoring v4 (evidence_score_v4)** — แก้ hash -9..+15 + เลขชุดเดียว (LLM ห้ามออกเลข) + ตัด circular nudge · ตรวจยืนยันบั๊กแล้ว 11 ส.ค. · แผนคัดข้อ: docs/ai/plans/ener-scoring-v4.md (รอกบเคาะข้อที่เอา)
0.5 **แก้หน้ารายงาน HTML ตาม UX audit** (Snapsite 11 ส.ค. 2026 คะแนน 78/100 → แก้ 3 ตัวแรกขึ้น 86 · กบ: "เดี๋ยวค่อยกลับมาแก้") — เรียงตามแรง:
   - 🔴 เรดาร์ 7 แกน render เป็น SVG เปล่าบางขนาดจอ (ของหลักของรายงานหายทั้งกราฟ — เช็ค viewport/zoom ที่มันวาดไม่ออก)
   - เรดาร์แกนตัวหนังสือเล็กเกิน ต้องเลื่อนไปเทียบตาราง 7 ด้าน → ใส่ label สั้นบนกราฟ + ขยายพื้นที่กราฟ
   - เกรด B ลอยไม่มีสเกล → เพิ่ม legend สั้น (เช่น B = ดี อันดับ 3 จาก 5) ข้างคะแนน ให้จบใน scroll แรก
   - แถวอันดับในคลังสูง <24px → เพิ่ม padding ให้ tap ได้ ≥44px (WCAG 2.5.8)
   - ตัวหนังสือทองอ่อนบนพื้นขาว contrast ไม่ถึง 4.5:1 → เข้มขึ้นหรือเปลี่ยนพื้น
   - jump-nav 01–07 เลขล้วน → ใส่ชื่อด้านย่อ + aria-label + เอาเลขเข้าไปใน h2
   - ปีปนกัน ค.ศ./พ.ศ. ในหน้าเดียว → เลือกอันเดียวทั้งรายงาน
   - SEO/แชร์: meta description (~150 ตัวอักษร) + JSON-LD Organization/WebSite + favicon (แชร์การ์ด og:image มีแล้ว ผ่าน)

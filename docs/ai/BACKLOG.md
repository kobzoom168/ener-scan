...
- [ ] objectInfoGate: path `SCAN_SHA256_DEDUP_HIT` (outbound ไม่มี report payload) และ `hasInfoForObject=true` ออกจาก gate ก่อน `consumeJobPreScanInfo` → ข้อมูลที่ลูกค้าพิมพ์ก่อนรูปค้างใน pre_job จนหมดอายุ (smoke 28 ส.ค. job aa15e0ab "หลวงพ่อคูณวัดบ้านไร่") — ควร consume+บันทึก (หรือ DEL พร้อม telemetry PRE_SCAN_OBJECT_INFO_DROPPED) ทุกทางออกของ gate · แยกรอบ ไม่ขยาย patch flow-role

...
## เส้น 401 ของการยืนยันเจ้าของ (Codex รอบ 7 — `9264e52`)
ปัญหา: `api()` ของหน้า LIFF ดัก 401 → `liff.login()` + Promise ไม่จบ → owner flow ไม่เคยได้ 401 ไปแสดงปุ่มลองอีกครั้ง และ boot ล้าง `liffReauth` ทุกครั้ง → เสี่ยงวนล็อกอิน
แก้เฉพาะเส้นยืนยัน: ย้าย `api()` ตัวจริงเป็น `createLiffApi()` ในโมดูล `liffOwnerVerify.client.js` (หน้า LIFF ฝังตัวเดียวกัน `var api = createLiffApi(...)`) เพิ่ม `opts.noReauth` — `runOwnerVerifyBoot()` เรียกด้วย `noReauth:true` เสมอ → 401 ถึง flow → "ยืนยันไม่สำเร็จ กรุณาลองใหม่ (เปิดหน้านี้จาก LINE อีกครั้ง)" + ปุ่มลองอีกครั้ง · ไม่ `liff.login()` เอง ไม่ตั้ง `liffReauth` ไม่ค้าง · หน้าจ่ายเงิน/หน้าอื่นยังได้ re-login หนึ่งรอบตามเดิม
เทสต์ (ผ่าน helper + boot ตัวจริง, fetch จริงไปเซิร์ฟเวอร์ ไม่ฉีด request ข้าม interceptor): 401 → login_expired, `liff.login` ไม่ถูกเรียก, `liffReauth` ไม่ถูกตั้ง, ไม่ redirect, Promise จบ (timeout guard) · กดลองอีกครั้งยังไม่ auto-login · happy path token ดี → cookie จริง → กลับ `/library` เห็นคลัง · หน้าอื่น 401 → `liff.login` 1 ครั้ง + `liffReauth` (พฤติกรรมเดิม) · หน้า LIFF ฝัง helper/boot ตัวเดียวกัน — `tests/ownerVerify.browserflow.test.js` **12/12**
